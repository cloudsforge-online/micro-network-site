 ...
 * (`authoriseRead`, `indexer/src/server.ts:708-717`) and the three faucet routes are unauthenticated
 ...
 * (`indexer/src/server.ts:711`), so an expired token would turn a public page into a 401 — a
 ...
 * `errorReply()` in every service (`indexer/src/server.ts:790-792`,
...
export interface ErrorNotice {
   ...
   * transaction" from "no such route" by code alone and says so at `indexer/src/server.ts:433-435`
