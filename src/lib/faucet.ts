 ...
 * `define(method, path, handler)` calls inside `buildRoutes()` (`faucet/src/server.ts`),
 * one per entry, and the `define` helper is at `faucet/src/server.ts`. So the citation for
 ...
 *
 *   * `OPTIONS /v1/drips` (`faucet/src/server.ts`) — the CORS preflight. It is issued **by the
 ...
 *   * `GET /metrics` (`faucet/src/server.ts`) — gated, and gated on purpose. `authorise` runs
 *     first (`faucet/src/server.ts`) and the service says why: "an open /metrics
 ...
 *     (`faucet/src/server.ts`), and a browser bundle holds neither and must not.
 *   * `GET /livez` (`faucet/src/server.ts`) and `GET /readyz` (`faucet/src/server.ts`) —
 ...
 *
 * Not by omission. `faucet/src/server.ts` — "Unauthenticated: a testnet faucet whose terms
 * require a credential to read is a faucet nobody can use" — for the poll: "it holds
 ...
 * (`faucet/src/server.ts`), and `requests.ts` states the rule in the
 ...
 * `idempotencyKey` (`faucet/src/server.ts`) — and there is no `Idempotency-Key` header
 * anywhere in that repository. `micro-trade` requires the header on every mutating route
 * (`trade/src/server.ts`), so the two clients look alike and are not interchangeable;
...
/**
 * The faucet's own description of itself — `GET /v1/faucet` (`faucet/src/server.ts`).
 *
 * **`dripWei`, `budgetRemainingWei` and every other amount are DECIMAL STRINGS, never numbers**,
 * and the service says why at `faucet/src/server.ts`: "1e19 wei is past what JSON's number
...
export interface FaucetTerms {
  /** Always the string `testnet` (`faucet/src/server.ts`). Not a variable. */
  readonly network: string
  /**
   * 7412. Read from `@cloudsforge/contracts-chain`, not from an environment variable
   * (`faucet/.env.example`): "This service dispenses on 7412 and there is no value of any
   * variable that changes that."
   */
  readonly chainId: number
  readonly asset: string
  /** Public the moment the first drip lands, and an operator topping up needs it. */
   ...
   *
   * `faucet/src/server.ts` — "Testnet EMBER. It has no value, it is not tradeable, and the
...
/** What `POST /v1/drips` answers (`faucet/src/server.ts`). */
export interface DripAccepted {
  ...
  /** The service's own poll path, e.g. `/v1/drips/<uuid>` (`faucet/src/server.ts`). */
  readonly poll: string
}

/**
 * One dispense, for the caller that is polling (`faucet/src/server.ts`).
 *
 * Note what is NOT here, and it is not an omission in this file: `readDispense` does not select
 * `raw_tx`, `nonce` or `custody_audit_id` (`faucet/src/requests.ts`). "The raw transaction
...
export interface DripView {
  readonly id: string
  readonly recipient: string
  /** `faucet/src/server.ts` enumerates the six: queued, signing, signed, broadcast, confirmed, failed. */
  readonly status: string
  readonly amountWei: string
  readonly txHash: string | null
  readonly confirmations: number
  /** A decimal string or null — a block number is a `bigint` in the store (`requests.ts`). */
 ...
 *
 * `faucet/src/server.ts` is the list the service scrapes a gauge for, and
 ...
 * at INFO and says so (`faucet/src/server.ts`) — and a page that painted all five red would
 * teach a reader that a correct answer is a failure. Three come from `RefusalCode`
 * (`faucet/src/limits.ts`) and two from `acceptDrip` (`faucet/src/requests.ts`).
 *
 * The MESSAGE is always the service's own and is always shown. `faucet/src/server.ts`:
...
/**
 * `GET /v1/faucet` — `faucet/src/server.ts`.
 *
...
/**
 * `POST /v1/drips` — `faucet/src/server.ts`.
 *
 * **202, not 200, and that is the contract.** The request is QUEUED — nothing has been signed and
 * no nonce has been read — and the caller polls `GET /v1/drips/:id` (`faucet/src/server.ts`).
 * A retry the service recognises answers **200** with `duplicate: true`, which is a
 ...
 * the recipient and the cooldown window (`faucet/src/requests.ts`), so two clicks inside one
...
export function requestDrip(
  ...
  return publicCall<DripAccepted>('/v1/drips', {
    ...
    // `faucet/src/server.ts` — "NOTHING IN THE BODY EXCEPT `address` AND `idempotencyKey`
...
/**
 * `GET /v1/drips/:id` — `faucet/src/server.ts`.
 *
 * Unauthenticated. An id that is not a v4 UUID is a **404** rather than a database error, and the
 * bound is applied before the query (`faucet/src/requests.ts`) so a caller-controlled string
