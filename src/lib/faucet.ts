 ...
 * `define(method, path, handler)` calls inside `buildRoutes()` (`faucet/src/server.ts:305-445`),
 * one per entry, and the `define` helper is at `faucet/src/server.ts:301-305`. So the citation for
 ...
 * | ------ | ---------------- | ----------------------- | ------------------------ |
 * | GET    | /v1/faucet       | none                    | faucet/src/server.ts:348 |
 * | POST   | /v1/drips        | none                    | faucet/src/server.ts:382 |
 * | GET    | /v1/drips/:id    | none                    | faucet/src/server.ts:418 |
 *
 * ── FOUR routes are declined, and none of them is a decision this page could take anyway ──────
 *
 *   * `OPTIONS /v1/drips` (`faucet/src/server.ts:439`) — the CORS preflight. It is issued **by the
 ...
 *     (`faucet/src/server.ts:441`).
 *   * `GET /metrics` (`faucet/src/server.ts:322`) — gated, and gated on purpose. `authorise` runs
 *     first (`faucet/src/server.ts:325`) and the service says why at `:316-317`: "an open /metrics
 ...
 *     (`faucet/src/server.ts:518-529`), and a browser bundle holds neither and must not.
 *   * `GET /livez` (`faucet/src/server.ts:315`) and `GET /readyz` (`faucet/src/server.ts:317`) —
 ...
 *
 * Not by omission. `faucet/src/server.ts:341-342` — "Unauthenticated: a testnet faucet whose terms
 ...
 * (`faucet/src/server.ts:379-380`, `:385-390`), and `requests.ts:126-131` states the rule in the
 ...
 * `idempotencyKey` (`faucet/src/server.ts:393-395`) — and there is no `Idempotency-Key` header
...
/**
 * The faucet's own description of itself — `GET /v1/faucet` (`faucet/src/server.ts:348-370`).
 *
 * **`dripWei`, `budgetRemainingWei` and every other amount are DECIMAL STRINGS, never numbers**,
 * and the service says why at `faucet/src/server.ts:357-358`: "1e19 wei is past what JSON's number
...
export interface FaucetTerms {
  /** Always the string `testnet` (`faucet/src/server.ts:353`). Not a variable. */
   ...
   *
   * `faucet/src/server.ts:365-367` — "Testnet EMBER. It has no value, it is not tradeable, and the
...
/** What `POST /v1/drips` answers (`faucet/src/server.ts:400-410`). */
export interface DripAccepted {
  ...
  /** The service's own poll path, e.g. `/v1/drips/<uuid>` (`faucet/src/server.ts:408`). */
  readonly poll: string
}

/**
 * One dispense, for the caller that is polling (`faucet/src/server.ts:421-434`).
 *
...
export interface DripView {
  readonly id: string
  readonly recipient: string
  /** `faucet/src/server.ts:465` enumerates the six: queued, signing, signed, broadcast, confirmed, failed. */
  readonly status: string
  readonly amountWei: string
 ...
 *
 * `faucet/src/server.ts:465` is the list the service scrapes a gauge for, and
 ...
 * at INFO and says so (`faucet/src/server.ts:257-259`) — and a page that painted all five red would
 ...
 *
 * The MESSAGE is always the service's own and is always shown. `faucet/src/server.ts:253-255`:
...
/**
 * `GET /v1/faucet` — `faucet/src/server.ts:348`.
 *
...
/**
 * `POST /v1/drips` — `faucet/src/server.ts:382`.
 *
 * **202, not 200, and that is the contract.** The request is QUEUED — nothing has been signed and
 * no nonce has been read — and the caller polls `GET /v1/drips/:id` (`faucet/src/server.ts:375-377`).
...
export function requestDrip(
  ...
  return publicCall<DripAccepted>('/v1/drips', {
    ...
    // `faucet/src/server.ts:379-380` — "NOTHING IN THE BODY EXCEPT `address` AND `idempotencyKey`
...
/**
 * `GET /v1/drips/:id` — `faucet/src/server.ts:418`.
 *
