...
const event = (n: number) => envelope({ app: 'explorer-web', type: 'T', message: `m${n}` })
...
describe('the envelope', () => {
  it('stamps the page the event came from', () => {
    ...
    assert.equal(wrapped.attributes['url'], 'https://explorer.cloudsforge.online/chains/ember/testnet')
    assert.equal(wrapped.route, '/chains/ember/testnet', 'the route is the column Lantern groups on')
    assert.equal(
  ...
  it('carries a kind from Lantern’s closed set, never a bare type', () => {
    installWindow('https://explorer.cloudsforge.online/')
    const wrapped = envelope({ app: 'explorer-web', type: 'NetworkError', message: 'boom' })
  ...
  it('rounds a duration to a whole millisecond, because value_ms is an INTEGER column', () => {
    installWindow('https://explorer.cloudsforge.online/')
    const wrapped = envelope({ app: 'explorer-web', type: 'PageLoad', message: '/', valueMs: 12.7 })
...
describe('the send', () => {
  it('posts to /ingest/client under a "samples" key', async () => {
    ...
    try {
      report({ app: 'explorer-web', type: 'NetworkError', message: 'boom' })
      await flush()
