...
# every route this bundle calls is anonymous — the chain read through `authoriseRead`
# (`indexer/src/server.ts:708-717`) and the three faucet routes by that service's own decision
# (`faucet/src/server.ts:334-335`) — and the client presents nothing. The rule survives the reason,
...
#
# It is fixed upstream now (`web-template/Dockerfile:39`), and this line was copied only after that
# file was opened and read — not on the strength of a sibling's comment saying so. Both
# `test/brand-chrome.test.ts` (which reads this file) and the image probe in ci.yml (which curls
# the running container for each asset) fail without it.
# ══════════════════════════════════════════════════════════════════════════════════════════════
COPY public ./public
...
# whether the chain index or the faucet behind the page is healthy, and on this surface it cannot
# even know whether either would answer this bundle at all. Pretending otherwise is how a green
# probe outlives a broken product.
